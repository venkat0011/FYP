...
clear;
close all;
data =  csvread('moving_signals/test3.csv');
% data= csvread('StationaryHuman_200.csv');
%% Converting the file into complex numbers
recieved_signal = data(:,1:141) +1j*data(:,142:end);
recieved_signal = recieved_signal(1660:end,:);
%% Clutter removal
alpha = 0.95; 
clutter_signal = zeros(length(recieved_signal),141);
clutter_signal(1,:) = recieved_signal(1,:);
for i = 2:length(recieved_signal) 
...
figure;
hold on
test = envelope(abs(clean_signal(300,:)));
test1 = envelope(abs(clean_suppressed(300,:)));
plot(test);
plot(test1);
hold off
legend('unsuppressed','suppressed')
figure;
imagesc(abs((clean_suppressed)));
...
buffer = 10;
step_size = 25;
seat_observation = zeros(floor(length(detected_signal)/step_size),2* length(seat_location)) ;
...
for i = 1:step_size:length(detected_signal)-step_size
    sub_array = abs(detected_signal(i:i+step_size,:)); % it will be a moving window of 5 seconds
