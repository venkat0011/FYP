...
% recieved_signal = recieved_signal(800:1400,:);
...
% figure;
% hold on
% test = (abs(clean_signal(300,:)));
% test1 = (abs(clean_suppressed(300,:)));
% plot(test);
% plot(test1);
% 
% hold off
% legend('unsuppressed','suppressed')

figure;
subplot(1,2,1);imagesc(envelope(abs(clean_suppressed)));axis on; title("with envelope");
subplot(1,2,2);imagesc(abs((clean_suppressed)));axis on; title("without envelope");
envelope_clean_signal=  envelope(abs(clean_suppressed));
envelope_clean_signal1 = abs(clean_suppressed);

%% distance compensation
envelope_clean_signal = envelope_clean_signal .*(1:141);
envelope_clean_signal1 = envelope_clean_signal1 .*(1:141);
% envelope_clean_signal_test = envelope_clean_signal .*(1:141);b223
% figure;
% imagesc((envelope_clean_signal));
% figure;
% imagesc((envelope_clean_signal1));
% subplot(1,2,1);imagesc(envelope_clean_signal);axis on; title("without compensation");
% subplot(1,2,2);imagesc(envelope_clean_signal_test);axis on; title("with compensation");
...
test_signal = Z_detect .* clean_signal;
test_signal = abs(test_signal);
Z = envelope_clean_signal1;
Z = Z .' ;
Z_detect = detector(Z,1:141);
Z_detect = Z_detect .' ;
detected_signal1 = Z_detect .* envelope_clean_signal1 ;

figure;
subplot(1,2,1);imagesc((abs(detected_signal)));axis on; title("with envelope");
subplot(1,2,2);imagesc(abs((detected_signal1)));axis on; title("without envelope");
figure;
subplot(1,2,1);imagesc((abs(detected_signal)));axis on; title("multiplied with envelope of clean signal");
subplot(1,2,2);imagesc(abs((test_signal)));axis on; title("multiplied with clean signal");

temp_array = zeros(count,1);
for k= 2:count
...
temp_array = zeros(count,1);
for k= 2:count
    [peaks,idx] = findpeaks((detected_signal(k,:)));
    temp_array(k,1) = length(peaks) ; % gives us the number of humans at each sample
end
number_of_peopleDetected = median(temp_array);
...
index_count = 1;
seat_location = [50]; % the seat location might not be exact detection so we have to give a leeway of 10%
max_seat_location = seat_location+10;
min_seat_location = seat_location-10;
buffer = 5;
step_size = 2;
rows = floor(count/step_size);
seat_observation = ones( rows ,2* length(seat_location)) .*3  ;
